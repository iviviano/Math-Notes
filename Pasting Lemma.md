...
```ad-proof
Let $D$ be [[Closed Set]] in $Y$. Then, pick $C_a$ [[Notation/subset|subset]] $A$ with $C_a = f$[[Notation/inverse|inverse]]$(D)$ with $C_a$ [[Closed Set]], and $C_a$ [[Notation/subset|subset]] $A$ with $C_b = f$[[Notation/inverse|inverse]]$(D)$ with $C_b$ [[Closed Set]]. Let $x\in h$[[Notation/inverse|inverse]]$(D)$. Then, if $x$ is in $A$, then, $h(x)= f(x)$, so $x\in C_a$. Otherwise, $x\in B$, so $g(x) = h(x)$, and $x\in C_b$. 
[[therefore]] $h$[[Notation/inverse|inverse]]$(D)\subseteq C_a\cup C_b$. 
